// <auto-generated />
using System;
using Clinicks.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Clinicks.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(ClinicksDbContext))]
    [Migration("20260611050923_AddRoom4AndCarlosLopez")]
    partial class AddRoom4AndCarlosLopez
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.26")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Clinicks.Domain.Entities.Cama", b =>
                {
                    b.Property<int>("NCama")
                        .HasColumnType("int")
                        .HasColumnName("n_cama");

                    b.Property<int>("IdHabitacion")
                        .HasColumnType("int")
                        .HasColumnName("id_habitacion");

                    b.Property<int>("IdEstado")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1)
                        .HasColumnName("id_estado");

                    b.HasKey("NCama", "IdHabitacion")
                        .HasName("PK__Cama");

                    b.HasIndex("IdEstado");

                    b.HasIndex("IdHabitacion");

                    b.ToTable("Cama", (string)null);

                    b.HasData(
                        new
                        {
                            NCama = 1,
                            IdHabitacion = 1,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 2,
                            IdHabitacion = 1,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 3,
                            IdHabitacion = 1,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 1,
                            IdHabitacion = 2,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 2,
                            IdHabitacion = 2,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 3,
                            IdHabitacion = 2,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 1,
                            IdHabitacion = 3,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 2,
                            IdHabitacion = 3,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 3,
                            IdHabitacion = 3,
                            IdEstado = 1
                        },
                        new
                        {
                            NCama = 1,
                            IdHabitacion = 4,
                            IdEstado = 2
                        },
                        new
                        {
                            NCama = 2,
                            IdHabitacion = 4,
                            IdEstado = 2
                        },
                        new
                        {
                            NCama = 3,
                            IdHabitacion = 4,
                            IdEstado = 2
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Ciudad", b =>
                {
                    b.Property<int>("IdCiudad")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_ciudad");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdCiudad"));

                    b.Property<int?>("IdProvincia")
                        .HasColumnType("int")
                        .HasColumnName("id_provincia");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("nombre");

                    b.HasKey("IdCiudad")
                        .HasName("PK__Ciudad");

                    b.HasIndex("IdProvincia");

                    b.ToTable("Ciudad", (string)null);

                    b.HasData(
                        new
                        {
                            IdCiudad = 1,
                            IdProvincia = 1,
                            Nombre = "CABA"
                        },
                        new
                        {
                            IdCiudad = 2,
                            IdProvincia = 1,
                            Nombre = "La Plata"
                        },
                        new
                        {
                            IdCiudad = 3,
                            IdProvincia = 2,
                            Nombre = "Córdoba Capital"
                        },
                        new
                        {
                            IdCiudad = 4,
                            IdProvincia = 3,
                            Nombre = "Rosario"
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Direccion", b =>
                {
                    b.Property<int>("IdDireccion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_direccion");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdDireccion"));

                    b.Property<int>("Altura")
                        .HasColumnType("int")
                        .HasColumnName("altura");

                    b.Property<string>("Calle")
                        .IsRequired()
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("calle");

                    b.Property<int?>("Dni")
                        .HasColumnType("int")
                        .HasColumnName("dni");

                    b.Property<int?>("IdCiudad")
                        .HasColumnType("int")
                        .HasColumnName("id_ciudad");

                    b.HasKey("IdDireccion")
                        .HasName("PK__Direccion");

                    b.HasIndex("Dni");

                    b.HasIndex("IdCiudad");

                    b.ToTable("Direccion", (string)null);
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.EstadoCama", b =>
                {
                    b.Property<int>("IdEstado")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_estado");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdEstado"));

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("nombre");

                    b.HasKey("IdEstado")
                        .HasName("PK_EstadoCama");

                    b.ToTable("EstadoCama", (string)null);

                    b.HasData(
                        new
                        {
                            IdEstado = 1,
                            Nombre = "Libre"
                        },
                        new
                        {
                            IdEstado = 2,
                            Nombre = "Ocupado"
                        },
                        new
                        {
                            IdEstado = 3,
                            Nombre = "En Mantenimiento"
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Habitacion", b =>
                {
                    b.Property<int>("IdHabitacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_habitacion");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdHabitacion"));

                    b.Property<string>("Nombre")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("nombre");

                    b.HasKey("IdHabitacion")
                        .HasName("PK__Habitacion");

                    b.ToTable("Habitacion", (string)null);

                    b.HasData(
                        new
                        {
                            IdHabitacion = 1,
                            Nombre = "Sala General"
                        },
                        new
                        {
                            IdHabitacion = 2,
                            Nombre = "Sala de Emergencias"
                        },
                        new
                        {
                            IdHabitacion = 3,
                            Nombre = "Terapia Intensiva"
                        },
                        new
                        {
                            IdHabitacion = 4,
                            Nombre = "Sala de Observación"
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Internacion", b =>
                {
                    b.Property<int>("IdInternacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_internacion");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdInternacion"));

                    b.Property<int?>("CreadoPorUsuarioId")
                        .HasColumnType("int")
                        .HasColumnName("creado_por_usuario_id");

                    b.Property<int?>("Dni")
                        .HasColumnType("int")
                        .HasColumnName("dni");

                    b.Property<DateTime?>("FechaEgreso")
                        .HasColumnType("datetime")
                        .HasColumnName("fecha_egreso");

                    b.Property<DateTime?>("FechaIngreso")
                        .HasColumnType("datetime")
                        .HasColumnName("fecha_ingreso");

                    b.HasKey("IdInternacion")
                        .HasName("PK__Internacion");

                    b.HasIndex("CreadoPorUsuarioId");

                    b.HasIndex("Dni");

                    b.ToTable("Internacion", (string)null);

                    b.HasData(
                        new
                        {
                            IdInternacion = 101,
                            CreadoPorUsuarioId = 1,
                            Dni = 50000001,
                            FechaIngreso = new DateTime(2026, 6, 10, 10, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            IdInternacion = 102,
                            CreadoPorUsuarioId = 1,
                            Dni = 50000002,
                            FechaIngreso = new DateTime(2026, 6, 10, 11, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            IdInternacion = 103,
                            CreadoPorUsuarioId = 1,
                            Dni = 50000003,
                            FechaIngreso = new DateTime(2026, 6, 10, 12, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.MovimientoCama", b =>
                {
                    b.Property<int>("IdMovimiento")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_movimiento");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdMovimiento"));

                    b.Property<DateTime?>("FechaFin")
                        .HasColumnType("datetime")
                        .HasColumnName("fecha_fin");

                    b.Property<DateTime>("FechaInicio")
                        .HasColumnType("datetime")
                        .HasColumnName("fecha_inicio");

                    b.Property<int>("IdHabitacion")
                        .HasColumnType("int")
                        .HasColumnName("id_habitacion");

                    b.Property<int>("IdInternacion")
                        .HasColumnType("int")
                        .HasColumnName("id_internacion");

                    b.Property<int>("NCama")
                        .HasColumnType("int")
                        .HasColumnName("n_cama");

                    b.HasKey("IdMovimiento")
                        .HasName("PK__MovimientoCama");

                    b.HasIndex("IdHabitacion");

                    b.HasIndex("IdInternacion");

                    b.HasIndex("NCama", "IdHabitacion");

                    b.ToTable("MovimientoCama", (string)null);

                    b.HasData(
                        new
                        {
                            IdMovimiento = 101,
                            FechaInicio = new DateTime(2026, 6, 10, 10, 0, 0, 0, DateTimeKind.Unspecified),
                            IdHabitacion = 4,
                            IdInternacion = 101,
                            NCama = 1
                        },
                        new
                        {
                            IdMovimiento = 102,
                            FechaInicio = new DateTime(2026, 6, 10, 11, 0, 0, 0, DateTimeKind.Unspecified),
                            IdHabitacion = 4,
                            IdInternacion = 102,
                            NCama = 2
                        },
                        new
                        {
                            IdMovimiento = 103,
                            FechaInicio = new DateTime(2026, 6, 10, 12, 0, 0, 0, DateTimeKind.Unspecified),
                            IdHabitacion = 4,
                            IdInternacion = 103,
                            NCama = 3
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Paciente", b =>
                {
                    b.Property<int>("Dni")
                        .HasColumnType("int")
                        .HasColumnName("dni");

                    b.Property<bool>("Activo")
                        .HasColumnType("bit");

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("apellido");

                    b.Property<int?>("CreadoPorUsuarioId")
                        .HasColumnType("int")
                        .HasColumnName("creado_por_usuario_id");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("nombre");

                    b.Property<string>("Telefono")
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)")
                        .HasColumnName("telefono");

                    b.HasKey("Dni")
                        .HasName("PK__Paciente__D87608A6A9A48A2E");

                    b.HasIndex("CreadoPorUsuarioId");

                    b.ToTable("Paciente", (string)null);

                    b.HasData(
                        new
                        {
                            Dni = 45678912,
                            Activo = true,
                            Apellido = "Lopez",
                            CreadoPorUsuarioId = 1,
                            Nombre = "Carlos",
                            Telefono = "1199887766"
                        },
                        new
                        {
                            Dni = 33445566,
                            Activo = true,
                            Apellido = "Fernández",
                            CreadoPorUsuarioId = 1,
                            Nombre = "Laura",
                            Telefono = "1144556677"
                        },
                        new
                        {
                            Dni = 22334455,
                            Activo = true,
                            Apellido = "Martínez",
                            CreadoPorUsuarioId = 1,
                            Nombre = "Diego",
                            Telefono = "1166778899"
                        },
                        new
                        {
                            Dni = 50000001,
                            Activo = true,
                            Apellido = "García",
                            CreadoPorUsuarioId = 1,
                            Nombre = "Ana",
                            Telefono = "1122223333"
                        },
                        new
                        {
                            Dni = 50000002,
                            Activo = true,
                            Apellido = "Díaz",
                            CreadoPorUsuarioId = 1,
                            Nombre = "Bruno",
                            Telefono = "1144445555"
                        },
                        new
                        {
                            Dni = 50000003,
                            Activo = true,
                            Apellido = "Torres",
                            CreadoPorUsuarioId = 1,
                            Nombre = "Cecilia",
                            Telefono = "1166667777"
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Pais", b =>
                {
                    b.Property<int>("IdPais")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_pais");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdPais"));

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("nombre");

                    b.HasKey("IdPais")
                        .HasName("PK__Pais");

                    b.ToTable("Pais", (string)null);

                    b.HasData(
                        new
                        {
                            IdPais = 1,
                            Nombre = "Argentina"
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Provincia", b =>
                {
                    b.Property<int>("IdProvincia")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id_provincia");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("IdProvincia"));

                    b.Property<int?>("IdPais")
                        .HasColumnType("int")
                        .HasColumnName("id_pais");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("nombre");

                    b.HasKey("IdProvincia")
                        .HasName("PK__Provincia");

                    b.HasIndex("IdPais");

                    b.ToTable("Provincia", (string)null);

                    b.HasData(
                        new
                        {
                            IdProvincia = 1,
                            IdPais = 1,
                            Nombre = "Buenos Aires"
                        },
                        new
                        {
                            IdProvincia = 2,
                            IdPais = 1,
                            Nombre = "Córdoba"
                        },
                        new
                        {
                            IdProvincia = 3,
                            IdPais = 1,
                            Nombre = "Santa Fe"
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Usuario", b =>
                {
                    b.Property<int>("UsuarioId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("usuario_id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("UsuarioId"));

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("apellido");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("email");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("nombre");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("password");

                    b.Property<int>("Rol")
                        .HasColumnType("int")
                        .HasColumnName("rol");

                    b.HasKey("UsuarioId")
                        .HasName("PK__Usuario__2ED7D2AF6D9861A0");

                    b.HasIndex(new[] { "Email" }, "UQ__Usuario__AB6E6164028903F3")
                        .IsUnique();

                    b.ToTable("Usuario", (string)null);

                    b.HasData(
                        new
                        {
                            UsuarioId = 1,
                            Apellido = "Admin",
                            Email = "admin",
                            Nombre = "Admin",
                            Password = "$2a$11$/04QedE4bTOHKu/ioEKvWeNrBtjl/Z21bysQIMhWUXjZ3E49vTFlC",
                            Rol = 1
                        });
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Cama", b =>
                {
                    b.HasOne("Clinicks.Domain.Entities.EstadoCama", "EstadoNavigation")
                        .WithMany("Camas")
                        .HasForeignKey("IdEstado")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_Cama_EstadoCama");

                    b.HasOne("Clinicks.Domain.Entities.Habitacion", "HabitacionNavigation")
                        .WithMany("Camas")
                        .HasForeignKey("IdHabitacion")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__Cama__id_habitac");

                    b.Navigation("EstadoNavigation");

                    b.Navigation("HabitacionNavigation");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Ciudad", b =>
                {
                    b.HasOne("Clinicks.Domain.Entities.Provincia", "ProvinciaNavigation")
                        .WithMany("Ciudades")
                        .HasForeignKey("IdProvincia")
                        .HasConstraintName("FK__Ciudad__id_provi");

                    b.Navigation("ProvinciaNavigation");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Direccion", b =>
                {
                    b.HasOne("Clinicks.Domain.Entities.Paciente", "PacienteNavigation")
                        .WithMany("Direcciones")
                        .HasForeignKey("Dni")
                        .HasConstraintName("FK_Direccion_Paciente");

                    b.HasOne("Clinicks.Domain.Entities.Ciudad", "CiudadNavigation")
                        .WithMany("Direcciones")
                        .HasForeignKey("IdCiudad")
                        .HasConstraintName("FK__Direccion__id_ci");

                    b.Navigation("CiudadNavigation");

                    b.Navigation("PacienteNavigation");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Internacion", b =>
                {
                    b.HasOne("Clinicks.Domain.Entities.Usuario", "CreadoPorUsuario")
                        .WithMany()
                        .HasForeignKey("CreadoPorUsuarioId")
                        .HasConstraintName("FK_Internacion_Usuario");

                    b.HasOne("Clinicks.Domain.Entities.Paciente", "PacienteNavigation")
                        .WithMany("Internaciones")
                        .HasForeignKey("Dni")
                        .HasConstraintName("FK__Internacion__dni");

                    b.Navigation("CreadoPorUsuario");

                    b.Navigation("PacienteNavigation");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.MovimientoCama", b =>
                {
                    b.HasOne("Clinicks.Domain.Entities.Habitacion", "HabitacionNavigation")
                        .WithMany()
                        .HasForeignKey("IdHabitacion")
                        .IsRequired()
                        .HasConstraintName("FK__MovimientoCama__id_habitacion");

                    b.HasOne("Clinicks.Domain.Entities.Internacion", "InternacionNavigation")
                        .WithMany("MovimientosCama")
                        .HasForeignKey("IdInternacion")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__MovimientoCama__id_internacion");

                    b.HasOne("Clinicks.Domain.Entities.Cama", "CamaNavigation")
                        .WithMany("MovimientosCama")
                        .HasForeignKey("NCama", "IdHabitacion")
                        .IsRequired()
                        .HasConstraintName("FK__MovimientoCama__cama");

                    b.Navigation("CamaNavigation");

                    b.Navigation("HabitacionNavigation");

                    b.Navigation("InternacionNavigation");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Paciente", b =>
                {
                    b.HasOne("Clinicks.Domain.Entities.Usuario", "CreadoPorUsuario")
                        .WithMany()
                        .HasForeignKey("CreadoPorUsuarioId")
                        .HasConstraintName("FK_Paciente_Usuario");

                    b.Navigation("CreadoPorUsuario");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Provincia", b =>
                {
                    b.HasOne("Clinicks.Domain.Entities.Pais", "PaisNavigation")
                        .WithMany("Provincias")
                        .HasForeignKey("IdPais")
                        .HasConstraintName("FK__Provincia__id_pa");

                    b.Navigation("PaisNavigation");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Cama", b =>
                {
                    b.Navigation("MovimientosCama");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Ciudad", b =>
                {
                    b.Navigation("Direcciones");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.EstadoCama", b =>
                {
                    b.Navigation("Camas");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Habitacion", b =>
                {
                    b.Navigation("Camas");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Internacion", b =>
                {
                    b.Navigation("MovimientosCama");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Paciente", b =>
                {
                    b.Navigation("Direcciones");

                    b.Navigation("Internaciones");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Pais", b =>
                {
                    b.Navigation("Provincias");
                });

            modelBuilder.Entity("Clinicks.Domain.Entities.Provincia", b =>
                {
                    b.Navigation("Ciudades");
                });
#pragma warning restore 612, 618
        }
    }
}
